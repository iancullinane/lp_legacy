...
/*
 * JS for the little phone icon popout thingy
 */
      ...
      $('.sticky').click(function()
          {
              if (!expanded)
              {
                  $(this).animate({'right' : '0px'}, {duration : 400});
                  expanded = true;
              }
              else
              {
                 $(this).animate({'right' : '-360px'}, {duration: 400});
                  expanded = false;
              }
          });
  });
...
/*
 * Unused JS for the map to be loaded
 */
...
function LandingAnimations() {
    ...
    jQuery('.phone, #about-two').waypoint(function (event) {
        jQuery(this).each(function (index) {
            var jQuerythis = jQuery(this);
            jQuerythis.delay(350 * (index + 1)).queue(function () {
                jQuerythis.addClass('animated fadeInLeft');
